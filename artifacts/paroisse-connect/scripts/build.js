...
function patchIndexHtml() {
  ...
  if (!fs.existsSync(indexPath)) {
    console.log("No index.html found to patch.");
    return;
  }

  let html = fs.readFileSync(indexPath, "utf-8");

  const CANONICAL    = "https://parish-connect-chat--mvira891.replit.app";
  const FULL_TITLE   = "Paroisse Connect \u2013 Application des paroisses catholiques";
  ...
  const schemaOrg = JSON.stringify({
    ...
    "@graph": [
      ...
      },
      {
        ...
        name: "Paroisse Connect",
        url: CANONICAL,
        description: "Application mobile et web pour les paroisses catholiques en France",
      },
    ],
  });
  ...
  // Fix <title> to use full SEO title
  html = html.replace(/<title>[^<]*<\/title>/, `<title>${FULL_TITLE}</title>`);

  // Inject SEO block right after <head> (fresh export — no prior patch expected)
  ...
  console.log("Patched index.html with full SEO block.");
}
...
function copyPublicFiles() {
  ...
  for (const entry of fs.readdirSync(publicDir)) {
    copyRecursive(path.join(publicDir, entry), path.join(webRoot, entry));
  }

  console.log("Copied public/ files to static-build/web/");
}
